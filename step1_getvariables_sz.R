...
recorded_file <- "20231226sz_category(Disease)_year(2019)_samplesize(10000).csv"
...
for (i in 1:nrow(traits)){
  ...
  if (is.null(exposure)) {extract_instruments(outcomes =x$id)}
    ...
    if(!is.null(exposure) && i!=j && x$population==y$population){
      tryCatch({
        mrresult <- mr(dat=harmonise_data(exposure_dat = exposure,
                                          outcome_dat =extract_outcome_data(snps = exposure$SNP, outcomes = y$id)),
